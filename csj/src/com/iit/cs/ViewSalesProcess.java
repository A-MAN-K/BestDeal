...
public class ViewSalesProcess extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		PrintWriter out = response.getWriter();
		String process 	= request.getParameter("process");
		int orderNumber	= Integer.parseInt(request.getParameter("orderNumber"));
		MySQLDataStoreUtilities dataStoreUtilities;
		...
		if(process.equals("update"))
		{
			...
			result = dataStoreUtilities.updateSalesOrder(orderNumber);
			
			if(result != 0 )
			{
				out.println("<tr><td>The Order Number "+orderNumber+"has been updated</td></tr>");
			}
			else
			{
		...
		{	
			int result = 0;
			dataStoreUtilities = new MySQLDataStoreUtilities();
			result = dataStoreUtilities.cancelOrder(orderNumber);
			
			if(result != 0)
			{
				out.println("<tr><td>The Order Number "+orderNumber+"has been deleted</td></tr>");
			}
			else
			{
